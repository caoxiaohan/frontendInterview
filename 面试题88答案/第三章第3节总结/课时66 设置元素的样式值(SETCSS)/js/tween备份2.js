~function () {
    ...
    function move(curEle, target, duration, effect, callBack) {
        ...
        //->effect:指定运动的公式的
        //->传递进来的值是数字:0-5
        //0->Linear 1->Circ-easeInOut 2->Elastic-easeOut 3->Back-easeOut 4->Bounce-easeOut 5->Expo-easeIn
        if (!isNaN(effect)) {
            effect = ["Linear", "Circ-easeInOut", "Elastic-easeOut", "Back-easeOut", "Bounce-easeOut", "Expo-easeIn"][effect];
            effect = effect.split("-");
            ...
            //->我们定义了五个形参,但是只传递四个值,最后一个值是一个回调函数->effect应该是没有值的,而effect这个形参存储的这个函数值本应该是callBack的值
            callBack = effect;
            effect = zhufengEffect.Linear;
        }

        //->callBack:当动画结束后我们可以做一些特殊的事情
        ...
        curEle.zhufengTimer = window.setInterval(function () {
            ...
            if (time >= duration) {
                ...
                window.clearInterval(curEle.zhufengTimer);
                //->动画运动完成后我们执行callBack,并且让回调函数中的this变为我们当前要处理的元素
                callBack && callBack.call(curEle);
                return;
            }
            for (key in target) {
                if (target.hasOwnProperty(key)) {
                    var cur = effect(time, begin[key], change[key], duration);
